...
public class PrayTime {
    ...
    // ------------------------------------------------------------
    // Calculation Methods
    final int Jafari = 0; // Ithna Ashari
    final int Karachi=1; // University of Islamic Sciences, Karachi
    final int ISNA=2; // Islamic Society of North America (ISNA)
    final int MWL=3; // Muslim World League (MWL)
    final int Makkah=4; // Umm al-Qura, Makkah
    final int Egypt=5; // Egyptian General Authority of Survey
    final int Tehran=6; // Institute of Geophysics, University of Tehran
    final int Custom=7; // Custom Setting

    // Juristic Methods
    final int Shafii=0; // Shafii (standard)
    final int Hanafi=1; // Hanafi
    // Adjusting Methods for Higher Latitudes
    final int None=0; // No adjustment
    final int MidNight=1; // middle of night
    final int OneSeventh=2; // 1/7th of night
    final int AngleBased=3; // angle/60th of night
    // Time Formats
    final int Time24=0; // 24-hour format
    final int Time12=1; // 12-hour format
    final int Time12NS=2; // 12-hour format with no suffix
    final int Floating=3; // floating point number
    // Time Names
    ...
    public PrayTime() {
        ...
        //InvalidTime = "0"; // The string used for invalid times
        ...
        methodParams.put(this.Jafari, Jvalues);

        // Karachi
        double[] Kvalues = {18,1,0,0,18};
        methodParams.put(this.Karachi, Kvalues);

        // ISNA
        double[] Ivalues = {15,1,0,0,15};
        methodParams.put(this.ISNA, Ivalues);

        // MWL
        double[] MWvalues = {18,1,0,0,17};
        methodParams.put(this.MWL, MWvalues);

        // Makkah
        double[] MKvalues = {18.5,1,0,1,90};
        methodParams.put(this.Makkah, MKvalues);

        // Egypt
        double[] Evalues = {19.5,1,0,0,17.5};
        methodParams.put(this.Egypt, Evalues);

        // Tehran
        double[] Tvalues = {17.7,0,4.5,0,14};
        methodParams.put(this.Tehran, Tvalues);

        // Custom
        double[] Cvalues = {18,1,0,0,17};
        methodParams.put(this.Custom, Cvalues);
    ...
    private void setCustomParams(double[] params) {
        ...
        for (int i = 0; i < 5; i++) {
            if (params[i] == -1) {
                ...
                methodParams.get(this.Custom)[i] = params[i];
            }
        }
        this.setCalcMethod(this.Custom);
    }
    ...
    private ArrayList<String> adjustTimesFormat(double[] times) {
        ...
        for (int i = 0; i < 7; i++) {
            if (this.getTimeFormat() == this.Time12) {
                result.add(floatToTime12(times[i], false));
            } else if (this.getTimeFormat() == this.Time12NS) {
